...
class SchedulerReportClient(object):
    ...
    def _clear_provider_cache_for_tree(self, rp_uuid):
        ...
        nova/ironic nodes or sharing providers) which should be unaffected by
        the error.
    ...
    def _create_client(self):
        ...
        client, ks_filter = clients.OpenStackClients(self._context).placement()
    ...
    def get_allocation_candidates(self, context, resources):
        ...
        :param zun.scheduler.utils.ResourceRequest resources:
        ...
        if (self._provider_tree.exists(uuid) and
            ...
            if not rps_to_refresh:
                try:
                    ...
                    warn_limit(
                        self,
                        'cannot establish connection to placement: ' + str(e))
                    # If we can't establish a connection to the
                    # placement service, like if placement isn't running or
                    # zun-compute is mis-configured for authentication, we'll
                    # get None back and need to treat it like we couldn't
                    # create the provider (because we couldn't).
    ...
    def _remove_resources_from_container_allocation(
        ...
        if not current_allocs['allocations']:
            ...
            raise exception.AllocationUpdateFailed(
                consumer_uuid=consumer_uuid,
                error=error_reason % {'resources': resources,
