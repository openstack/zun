...
from zun.common import utils as zun_utils
import zun.conf
from zun.scheduler.client import report
from zun.scheduler import utils as scheduler_utils
from zun.tests import base
from zun.tests.unit import fake_requests
...
class SchedulerReportClientTestCase(base.TestCase):

    def setUp(self):
        super(SchedulerReportClientTestCase, self).setUp()
        self.context = context.get_admin_context()
        self.ks_adap_mock = mock.Mock()
        self.compute_node = utils.get_test_compute_node(
            self.context,
        ...
        with zun_utils.nested_contexts(
            mock.patch(
                'keystoneauth1.loading.adapter.Adapter.load_from_options',
                return_value=self.ks_adap_mock),
            mock.patch('zun.common.clients.OpenStackClients.keystone')
        ):
...
class TestPutAllocations(SchedulerReportClientTestCase):
    ...
    def test_remove_provider_from_inst_alloc_no_shared(self):
        ...
        project_id = uuids.project_id
        user_id = uuids.user_id
        res = self.client.remove_provider_tree_from_container_allocation(
    ...
    def test_remove_provider_from_inst_alloc_no_source(self):
        """Tests that if remove_provider_tree_from_container_allocation() fails
    ...
    def test_remove_provider_from_inst_alloc_fail_get_allocs(self):
        ...
        self.assertRaises(
            ...
            self.client.remove_provider_tree_from_container_allocation,
            self.context, consumer_uuid, uuids.source)
...
class TestMoveAllocations(SchedulerReportClientTestCase):
    ...
    def setUp(self):
        ...
        patcher = mock.patch(
            'zun.scheduler.client.report.SchedulerReportClient.post')
    ...
    @mock.patch('zun.scheduler.client.report.LOG.info')
    ...
    def test_move_to_non_empty_target(self):
        ...
        with mock.patch('zun.scheduler.client.report.LOG') as mock_log:
            resp = self.client.move_allocations(
    ...
    @mock.patch('zun.scheduler.client.report.LOG.warning')
...
class TestProviderOperations(SchedulerReportClientTestCase):
    @mock.patch('zun.scheduler.client.report.SchedulerReportClient.'
    ...
    def test_ensure_resource_provider_get(
            self, get_rpt_mock, get_shr_mock, get_trait_mock, get_agg_mock,
            get_inv_mock, create_rp_mock):
    ...
    @mock.patch('zun.scheduler.client.report.SchedulerReportClient.'
                'get_providers_in_tree')
    def test_ensure_resource_provider_create_fail(
    ...
    @mock.patch('zun.scheduler.client.report.SchedulerReportClient.'
                '_refresh_associations')
    def test_ensure_resource_provider_refresh_create(
            self, mock_refresh, mock_create, mock_gpit):
    ...
    def test_refresh_associations_time(self, log_mock):
        ...
        with mock.patch('time.time') as mock_future:
            ...
            mock_future.return_value = (
                now + CONF.compute.resource_provider_association_refresh / 2)
            self.client._refresh_associations(self.context, uuid)
            self.assert_getters_not_called(timer_entry=uuid)

            # Called because time has passed.
            mock_future.return_value = (
                now + CONF.compute.resource_provider_association_refresh + 1)
            self.client._refresh_associations(self.context, uuid)
            self.assert_getters_were_called(uuid)

    def test_refresh_associations_disabled(self):
        """Test that refresh associations can be disabled."""
        CONF.set_override('resource_provider_association_refresh', 0,
                          group='compute')
        uuid = uuids.compute_node
        # Seed the provider tree so _refresh_associations finds the provider
        self.client._provider_tree.new_root('compute', uuid, generation=1)
...
class TestAllocations(SchedulerReportClientTestCase):
    ...
    @mock.patch("zun.scheduler.client.report.SchedulerReportClient."
                "delete")
    @mock.patch("zun.scheduler.client.report.SchedulerReportClient."
                "delete_allocation_for_container")
    @mock.patch("zun.objects.Container.list_by_host")
    def test_delete_resource_provider_cascade(
    ...
    @mock.patch("zun.scheduler.client.report.SchedulerReportClient."
                "delete")
    @mock.patch('zun.scheduler.client.report.LOG')
...
class TestAggregateAddRemoveHost(SchedulerReportClientTestCase):
    ...
    from the zun API, not the zun compute manager/resource tracker.
    """
    def setUp(self):
        super(TestAggregateAddRemoveHost, self).setUp()
        self.mock_get = self.useFixture(
            fixtures.MockPatch('zun.scheduler.client.report.'
