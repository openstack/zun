...
base_op_count=`grep "op\." -R zun/db/sqlalchemy/alembic/versions/ | wc -l`
baseline_report=$(mktemp -t zun_coverageXXXXXXX)
...
current_op_count=`grep "op\." -R zun/db/sqlalchemy/alembic/versions/ | wc -l`
current_report=$(mktemp -t zun_coverageXXXXXXX)
find . -type f -name "*.pyc" -delete && python setup.py testr --coverage --testr-args="$*"
